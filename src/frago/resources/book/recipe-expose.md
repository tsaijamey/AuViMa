...
- 把结果预先算成文件写进平台交代的落点，页面只渲染 → 可以 public；
...
`/app/<name>/data/…` 给的是本机自己那次运行的落点，页面调 `api/<mode>` 也以同一个落点作答。
...
- 数据要给**别的配方**读 → 写进 `~/.frago/recipe-data/<配方>/share/common/`，
  然后两边各说一句：被读的一方写 `shares: share/common`（我交出自己数据里的哪一块），
  读的一方写 `reads_common: [<生产者>]`。两句缺一句就拿不到，
  `frago recipe validate` **直接拒绝**并指出缺的是哪一句。
  不需要任何人再授权：平台只交出声明的那一块，且在隔离下物理只读——
  有界且真只读的东西，机器自己就能放行。
...
反过来的契约——配方声明什么是公开的，别的一概不出去：
    ...
    publish("my_recipe", {
        "files": ["q3.png"],                      # 私有：只有主人的 config.json 看得到
        "public": {"title": "Q3 numbers"},        # 只有这个出去
    })

**路径一个都不许有**：`publish` 里出现本机位置会当场报错，导出 mode 的返回值里出现
也会被拒。页面那一侧没有这台机器的文件系统，而同一条路径在主人机器上指主人的目录，
别人打开时会把主人的文件原样送出去、页面还渲染得好好的。

落点底下的文件仍从 `/app/<name>/data/<文件>` 可读，**目录在哪由服务端按请求者自己算**——
主人和「读配方自己那份」的人拿到本机那一份，各读各份的登录用户拿到自己名下那一份，
和运行时写哪儿是同一个判断。数据分项目的配方，页面按 `data/projects/<项目名>/<文件>` 取。

非主人侧 `config.json` 带 `apiBase: null`、`readOnly: true`，
...
- 不要以为发布一个 slot 等于发布全部；不要把敏感数据写进配方的落点，那个目录整体可读。
