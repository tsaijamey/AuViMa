...
3. **页面上要不要有按钮？** 这一问 expose 回答不了：能按什么写在**配方**那个 mode 方法上
   （`@action`），由写配方的人逐个点名。`--runnable` 已经取消。
...
- **这几个人本来就该看配方自己算的那一份** → `frago recipe expose <name> --allow … --shared`。
  一条命令，不动数据，页面直接读配方自己的槽，文件从本机那次运行的落点取。
...
1. **一个 mode 一个访问级别。** `@export` 的按契约只读，页面直接 `POST /app/<名>/api/<mode>` 就能读，
   不需要按钮——所以「既 @export 又 @action」不是一种状态，写出来 `validate` 直接报错。
2. **产出 MUST 落在平台交代的目录里。** 建在基类上就自动满足（`self.data_dir` 就是那个读取，
   而且运行器在这次运行自己的目录里起进程）。会被拦的是绝对路径和 `Path.home()/…` 这类
   跟「谁在跑」无关的固定位置——它们平时只是警告，一旦有 mode 标了 `@action` 就升级成拦截，
   因为那时每个人按下去写的都是同一份。
3. **页面 MUST 在 body 里点名 mode**：`{"params": {"mode": "save", …}}`。不写 mode 一律 403。
   平台不替你填配方的默认 mode——默认是「没人点名时跑哪个」，而访问级别的全部意义就是
   被授权的那个就是跑的那个；替你填就是授权了一个 mode、跑了另一个。**这一条在作者自己机器上测不出来**：主人那条路根本不查声明，
   所以缺 mode 的页面在你机器上一切正常、只在服务器上以登录用户身份打开才坏。

**这是权限决定，不是格式转换。** 给一个 mode 标 `@action`，等于让**每一个能打开这张页面的人**
...
访问级别写在 `recipe.py` 的方法上，而配方住在 `~/.frago/recipes/`，**不在 frago 仓库里**。
...
配方标了 `@action` 的话 `/app/` 也要限速——那底下的 `POST .../run` 会在服务器上起进程。
平台自己那两层兜底挡的是并发，不是频率。
