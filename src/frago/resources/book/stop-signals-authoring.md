...
一轮活结束时，frago 要判断这一轮该不该就这么结束。判断分两层：先过一道不花钱的本地闸门，命中了才去问轻量 ai。闸门看什么，写在 `~/.frago/hook/stop-signals.json`，改完下一轮生效，不用重建引擎。
...
文件里另有一个 `discard_reason_regex`：轻量 ai 给出的拦截理由里出现这些措辞就整条丢弃。那句理由会留在会话记录里影响后面每一轮，不只是这一轮，所以「别问用户」这类话一个字都不能放进去。
