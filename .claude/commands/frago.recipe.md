...
# --- 交互操作 ---
# 点击元素
uv run frago click <selector> [--wait-timeout 10]
# 滚动页面 (正数向下，负数向上)
uv run frago scroll <pixels>
# 等待指定时间
...
# 截图 (加 --full-page 截全屏)
uv run frago screenshot <output_file> [--full-page]

# --- 视觉确认 (用于验证选择器) ---
# 高亮元素
uv run frago highlight <selector>
# 显示鼠标指针
...
你: 第一步需要做什么？
用户: 点击"作者声明"展开详情
你: [执行] uv run frago click '[aria-label="作者声明"]'
你: [等待] uv run frago wait 0.5
你: [验证] uv run frago screenshot /tmp/step1_result.png
你: [查看截图，确认详情已展开]
    ✓ 成功。记录：ARIA选择器（优先级5）
    ✓ 验证通过：页面出现了详情内容区域
    下一步呢？
...
1. **导航操作**（navigate/click导致页面跳转）
   - 验证URL是否变化：`uv run frago exec-js "window.location.href" --return-value`
   - 验证页面标题：`uv run frago get-title`
   - 验证关键元素出现：`uv run frago exec-js "document.querySelector('<特征元素>') !== null" --return-value`

2. **UI交互操作**（点击按钮/菜单）
   - 截图对比：`uv run frago screenshot /tmp/stepN.png`
   - 验证目标元素出现/消失
   - 检查文本内容变化
...
- 如<网站名>改版导致脚本失效，使用 `/frago.recipe update <配方名>` 更新
- <其他注意事项>
...
## 注意事项
- 依赖 Recipe 必须可用（检查: `uv run frago recipe list`）
...
如果用户执行 `/frago.recipe update <配方名> "原因"`：

1. **查找配方位置**：
   ```bash
   # 使用 recipe info 找到配方位置
   uv run frago recipe info <recipe_name> --format json
   ```
...
1. **导入 RecipeRunner**：`from frago.recipes import RecipeRunner`
...
根据用户输入判断模式：
- 如果是 `/frago.recipe create workflow "<任务描述>"`：进入 **Workflow 创建模式**
- 如果是 `/frago.recipe create "<任务描述>"`：进入 **Atomic 创建模式**
- 如果是 `/frago.recipe update <配方名> "原因"`：进入**更新模式**
- 如果是 `/frago.recipe list`：调用 `uv run frago recipe list`
