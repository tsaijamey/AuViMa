---
description: "执行一次性的复杂任务（使用完整的auvima工具集）"
---
...
作为任务执行者，你需要使用完整的 **auvima 工具集**完成用户指定的一次性复杂任务。与 `/auvima.run`（专注于调研）不同，本命令专注于**任务完成**。

## 核心定位

- **目标**：完成用户指定的具体任务（如"在Upwork申请5个Python职位"）
- **成功标准**：任务目标达成 + 结果可验证
- **工作空间**：使用 `projects/` 目录（与 `/auvima.run` 共享基础设施）
...
# 查看具体命令的用法
uv run auvima <command> --help

# 示例
uv run auvima navigate --help
...
**初始化和上下文**：
```bash
uv run auvima run init "task-description"
uv run auvima run set-context <project_id>
```
...
uv run auvima recipe info <name> --format json
...
**浏览器操作**：参考 `uv run auvima --help`，包括：
- `navigate` - 导航
- `click` - 点击
...
```bash
# 创建 project
uv run auvima run init "upwork python job apply"

# 设置上下文（假设返回的 project_id 是 upwork-python-job-apply）
uv run auvima run set-context upwork-python-job-apply
```
...
```bash
# 示例：导航到Upwork
uv run auvima navigate https://upwork.com/jobs
...
# 示例：提取职位列表
uv run auvima exec-js "Array.from(document.querySelectorAll('.job-tile')).map(el => ({title: el.querySelector('.title').textContent, url: el.querySelector('a').href}))"
...
```bash
# 发现现有 Recipe
uv run auvima recipe list --format json | grep "upwork"

# 如果有现成的 Recipe
uv run auvima recipe run upwork_apply_job \
...
```bash
# 保存关键截图
uv run auvima run screenshot "申请成功页面"
...
**🖼️ 多媒体文件**（通过工具生成）：
- `screenshots/*.png` - 截图（使用 `uv run auvima screenshot`）
...
- **禁止使用 `cd`**：会导致 `uv run auvima` 命令失效
- **上下文优先级**：环境变量 `AUVIMA_CURRENT_RUN` > 配置文件 `.auvima/current_project`
