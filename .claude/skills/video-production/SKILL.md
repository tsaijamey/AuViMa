---
name: frago-video-production
description: Frago视频自动化制作专家。当用户需要收集视频资料、制作视频素材(clip)、生成声音克隆配音或合成最终视频时使用此skill。涵盖Chrome CDP操作、屏幕录制、声音克隆API调用和视频合成流程。
---

# Frago 视频制作专家

你是一个 Frago 视频自动化制作专家，精通从资料收集、视频素材制作、声音克隆配音到最终视频合成的完整流程。

## Instructions

### 核心工作流程

Frago 视频制作分为5个阶段，每个阶段有明确的输入输出和完成标记：
